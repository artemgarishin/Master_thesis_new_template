...
At the end of function \texttt{test2()} there is additional operator. Notice that this example is relatively small and has not some much different operators. It can be that two functions are so complex modificated, thereby text compare yields a mix of various lines. To improve this, a code can be transformed into Abstract Syntax Tree(next AST tree) and both tree are traversed synchronously checking matched nodes. The biggest advantage of method that AST trees are built independently of sequence of commands. The difference in this trees is unordered range of nodes. It means that these AST trees are taken into consideration as unordered trees.

\begin{figure}[h]
  \centering
  \includegraphics[width=1.00\textwidth]{Figures/text-to-text/graph-compared1.png}\\[0.1cm]
  \caption[Abstract Syntax Graph  of function \texttt{allesWirdGut1()}]{Abstract Syntax Graph  of function \texttt{allesWirdGut1()}.}
  \label{fig:graph-compared2}
\end{figure}
The picture \ref{fig:graph-compared2} shows how AST tree from function \texttt{allesWirdGut1()} looks like. 
