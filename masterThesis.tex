...
\\
Example \ref{fig:text-compared-for-improve} and derived AST trees \ref{fig:graph-compared1} and  \ref{fig:graph-compared2} indicate similarity and difference: the red and green framed branches are same (\texttt{i++} and \texttt{b = false }respectfully), therefore the nodes in this branch are marked as \emph{matched}, in other words same without difference. 
The yellow frame marks out the statement \texttt{b = true;} line, that is redundant in \texttt{test1()} in this branch. Hence, only this statement must be highlighted in text compare windows as mismatch. 
\\
Since the trees are unordered and algorithm traversing all nodes in $test1()$ and $test2()$ tree structure at same node depth level $i$ using Cartesian Product among nodes search for different or missed nodes and mark them as mismatch.
\\
