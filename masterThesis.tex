...
\end{tabular}
\end{minipage}
\caption{The solution MBM of bipartite graph brings $5+4=9$}
\label{fig:ex1}
\end{figure}
%PIC 1
...
By the same way all entries in table \ref{fig:ex1} have been built. Passing through the tree $ T_{1}$ at another left branch until leaves $v_{2}$ and $w_{3}$ the similar bipartite graph can be formed \ref{fig:ex6}. As said before the current edges get same weight equal one. The edge $(v_{2},w_{8})$ is selected. Under those circumstances the next bipartite graph can be built \ref{fig:ex5}. Based on previous solution the edge $(v_{4},w_{9})$ obtains weight equal to two.
